...
export default function Choferes() {
  ...
  const [deleteChofer, setDeleteChofer] = useState(null);
  ...
  const choferesPorEmpresa = user?.rol === 'SuperAdmin'
    ? choferes
  ...
  const handleExport = () => {
    const dataToExport = filteredChoferes.map(c => ({
      ...
      ...(user?.rol === 'SuperAdmin' && { 'Empresa': c.empresa })
    }));
  ...
  const validate = () => {
    ...
    if (!formData.licencia.trim()) newErrors.licencia = 'La licencia es obligatoria';
    // ⭐ WhatsApp OBLIGATORIO para choferes (whitelist)
    if (!formData.whatsapp.trim()) {
      newErrors.whatsapp = 'El WhatsApp es obligatorio para la whitelist';
  ...
  const handleSave = () => {
    ...
    if (editingChofer) {
      setChoferes(choferes.map(c =>
        c.id === editingChofer.id ? { ...formData, id: c.id, empresaId: c.empresaId, empresa: c.empresa } : c
      ));
    } else {
      const newChofer = {
        ...formData,
  ...
  return (
    <Box>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 3 }}>
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Whitelist de WhatsApp para envío de eventos • {filteredChoferes.length} {filteredChoferes.length === 1 ? 'chofer' : 'choferes'}
            </Typography>
          </Box>
        </Box>

        <Alert severity="warning" sx={{ mb: 3 }}>
          <strong>Whitelist de WhatsApp:</strong> Solo los choferes registrados aquí con número de WhatsApp 
          podrán enviar eventos de combustible desde la aplicación móvil. Estos usuarios NO tienen acceso al frontoffice web.
        </Alert>
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}>
            ...
            <TextField
              ...
              helperText={errors.whatsapp || "Formato: +54 9 351 234 5678 - Obligatorio para enviar eventos"}
      ...
      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        ...
        <DialogContent>
          ¿Estás seguro de eliminar al chofer {deleteChofer?.nombre} {deleteChofer?.apellido}?
        </DialogContent>
        <DialogActions>
