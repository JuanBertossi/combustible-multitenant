...
export default function Surtidores() {
  const { user } = useAuth();
  const [surtidores, setSurtidores] = useState(mockSurtidores);
  const [openDialog, setOpenDialog] = useState(false);
  const [openDeleteDialog, setOpenDeleteDialog] = useState(false);
  const [editingSurtidor, setEditingSurtidor] = useState(null);
  const [deleteSurtidor, setDeleteSurtidor] = useState(null);
  ...
  const surtidoresPorEmpresa = user?.rol === 'SuperAdmin'
    ? surtidores
  ...
  const handleExport = () => {
    const dataToExport = filteredSurtidores.map(s => ({
      ...
      ...(user?.rol === 'SuperAdmin' && { 'Empresa': s.empresa })
    }));
  ...
  const handleSave = () => {
    if (!validate()) return;
    if (editingSurtidor) {
      setSurtidores(surtidores.map(s =>
        s.id === editingSurtidor.id ? { ...formData, id: s.id } : s
      ));
    } else {
      const newSurtidor = {
        ...formData,
  ...
  const handleDeleteClick = (surtidor) => {
  ...
  return (
    <Box>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 3 }}>
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Gestión de surtidores y puntos de carga • {filteredSurtidores.length} {filteredSurtidores.length === 1 ? 'surtidor' : 'surtidores'}
            </Typography>
          </Box>
        </Box>
      ...
      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        ...
        <DialogContent>
          ¿Estás seguro de eliminar el surtidor {deleteSurtidor?.codigo} - {deleteSurtidor?.nombre}?
        </DialogContent>
        <DialogActions>
